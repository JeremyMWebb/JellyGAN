...
from Networks_paper_s1 import Generator, Critic
from Support import precheck, plot_grid, plot_loss, to_grid
...
#    Gen_train.compile( RMSprop( lr=Learning_rate), loss= wass_loss)
    Gen_train.compile( Adam( lr=0.01, beta_1=0.00, beta_2=0.99), loss= wass_loss)
...
#    Crt_train.compile( RMSprop( lr=Learning_rate), loss= [wass_loss, wass_loss, partial_gp_loss], loss_weights=[1, 1, 10])
    Crt_train.compile( Adam( lr=0.01, beta_1=0.00, beta_2=0.99), loss= [wass_loss, wass_loss, partial_gp_loss], loss_weights=[1, 1, 10])
...
def critic_generator( Batch_size, Height, Width, Channels):
    data = np.load('JellyBW.npy', allow_pickle=True)
    while True:
        imgs = np.empty(( Batch_size, Width, Height, Channels), dtype='float32')
        for i in range( Batch_size):
            selection = np.random.randint( 0, len( data))
            hold = data[ selection]
            if np.random.ranf() < 0.5:
                hold = np.fliplr( hold)
            if np.random.ranf() < 0.3:
                hold = hold + np.random.uniform( -0.025, 0.025, size=1)
                
            hold = cv2.resize( hold, (Width, Height))
            imgs[i,:,:,0] = hold
        imgs = ( imgs -127.5) / 127.5
        yield imgs

def EquLearning( model):
    for layer in model.layers:
        weight = layer.get_weights()
        if len( weight) == 2:
            std = np.sqrt( 2./ np.prod( weight[0].shape[:-1]), dtype='float32')
            weight[0] *= std
...
################################  TRAIN CRITIC  ###############################
                ...
                crt_real.append( outC[1])
                crt_fake.append( outC[2])

#############################  TRAIN GENERATOR  ###############################
                z = np.random.randn( Batch_size, 512).astype('float32')
                outG = Gen_train.train_on_batch( z, Y)
                gen_loss.append( outG)
                print( outC)
                print( outG)
                
            Tcrt_real.append( np.mean( crt_real[-Steps_per_epoch:]))
            Tcrt_fake.append( np.mean( crt_fake[-Steps_per_epoch:]))
            Tgen_loss.append( np.mean( gen_loss[-Steps_per_epoch:]))
...
if __name__ == '__main__':
    ...
    dest_folder = 'Results_BW_jellies_s1_gamma_100'
    if not os.path.isdir( dest_folder):
        os.mkdir( dest_folder)

    train( Continue, init_block, final_block, dest_folder, Steps_per_epoch=Steps, Epochs=Epochs, Channels=Channels, Learning_rate=Learning_rate)
